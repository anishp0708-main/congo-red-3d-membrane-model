...
// Outer glass pipes and bright rims. These are intentionally high contrast.
...
const acCartridge = makeCartridge("Activated carbon", 0x14171d, 1.25, {
  opacity: 0.92,
  ringColor: 0x9ba3ae,
  labelColor: "#e8ecf4"
});

const chitosanCartridge = makeCartridge("Chitosan membrane", 0xf5deb2, -1.25, {
  opacity: 0.82,
  ringColor: 0xf1fff7,
  labelColor: "#bffff1"
});
...
function addAcPellets() {
  ...
  for (let i = 0; i < 95; i++) {
    const pellet = new THREE.Mesh(
      geo,
      new THREE.MeshBasicMaterial({ color: colors[i % colors.length], transparent: true, opacity: 0.94 })
    );
    pellet.position.set(
...
function addChitosanSitesAndFibers() {
  const fiberMat = new THREE.MeshBasicMaterial({ color: 0xffefc9, transparent: true, opacity: 0.78 });
  for (let i = 0; i < 38; i++) {
    ...
    const curve = new THREE.CatmullRomCurve3([
      ...
      new THREE.Vector3(0.35 + Math.random() * 0.15, yBase + (Math.random() - 0.5) * 0.18, zBase),
      new THREE.Vector3(0.75 + Math.random() * 0.16, yBase + (Math.random() - 0.5) * 0.18, zBase)
    ]);
  ...
  const siteMat = new THREE.MeshBasicMaterial({ color: 0x7fffd4, transparent: true, opacity: 0.96 });
  for (let i = 0; i < 70; i++) {
...
const dyeMaterial = new THREE.MeshBasicMaterial({ color: 0xff2447, transparent: true, opacity: 0.98, depthWrite: false });
const capturedDyeMaterial = new THREE.MeshBasicMaterial({ color: 0xd90036, transparent: true, opacity: 0.88, depthWrite: false });
const waterMaterial = new THREE.MeshBasicMaterial({ color: 0x59c8ff, transparent: true, opacity: 0.78, depthWrite: false });
...
function isInsideCaptureZone(pathType, pos) {
  const centerY = pathType === "ac" ? 1.25 : -1.25;
  return pos.x > -0.08 && pos.x < 1.15 && Math.abs(pos.y - centerY) < 0.28 && Math.abs(pos.z) < 0.22;
}
...
function computeResults(s) {
  ...
  return { R, chitosan, ac, f, removalFlow, CeFlow, zUntreated, zTreated, foldGain };
}
...
function updateSimulation() {
  ...
  const chitosanAdvantage = visual.acCe / Math.max(visual.chitosanCe, 0.000001);
  const acLeft = (visual.acRelative * 100).toFixed(1);
  const chLeft = (visual.chitosanRelative * 100).toFixed(1);
  ...
  document.getElementById("metricFlowSub").textContent = `${formatPct(results.f * 100, 0)} of chitosan equilibrium reached`;
  document.getElementById("metricFold").textContent = `${results.foldGain.toFixed(1)}x`;

  document.getElementById("acLine").textContent = `Outlet dye left: ${acLeft}% of inlet`;
  document.getElementById("chitosanLine").textContent = `Outlet dye left: ${chLeft}% of inlet`;
  document.getElementById("acCe").textContent = formatMgL(visual.acCe);
  document.getElementById("chitosanCe").textContent = formatMgL(visual.chitosanCe);

  document.getElementById("compareNote").textContent = state.visualMode === "equilibrium"
    ? `In equilibrium mode, the chitosan outlet has about ${chitosanAdvantage.toFixed(1)}x less dye than the activated-carbon outlet at these settings.`
    : "In flow mode, chitosan uses the contact-time model while activated carbon remains an equilibrium benchmark.";

  document.getElementById("modeNote").textContent = state.visualMode === "equilibrium"
  ...
  document.getElementById("scene-status").textContent = "3D pipe system loaded - drag scene to tilt";
...
function bindInputs() {
  ...
  ranges.forEach((input) => {
    input.addEventListener("input", (event) => setValue(input.dataset.key, event.target.value, input));
  });

  numbers.forEach((input) => {
    input.addEventListener("change", (event) => setValue(input.dataset.key, event.target.value, input));
    input.addEventListener("input", (event) => {
      if (event.target.value !== "") setValue(input.dataset.key, event.target.value, input);
    });
  });
...
function updateParticle(particle, dt, elapsed) {
  ...
  const swirlA = Math.sin(elapsed * 7.0 + data.t * 24 + (data.pathType === "ac" ? 1 : 3));
  const swirlB = Math.cos(elapsed * 6.0 + data.t * 18 + (data.pathType === "ac" ? 2 : 4));

  // Keep the Congo Red molecule inside the transparent pipe.
  // The pipe radius is about 0.25 scene units, so the cross-section offsets stay small.
...
function updateWaterParticle(particle, dt, elapsed) {
  ...
  // Water molecules pass through both cartridges easily and remain inside the pipe.
